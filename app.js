...
async function loadDashboard() {
  ...
  try {
    ...
    const totalMealsHistory =
      mealsRes.data?.reduce(
        (s, i) => s + (parseFloat(i.day_count) + parseFloat(i.night_count)),
        0,
      ) || 0;
...
async function loadProfile() {
  ...
  try {
    ...
    const totalUserMeals =
      userMeals.data?.reduce(
        (s, m) => s + (parseFloat(m.day_count) + parseFloat(m.night_count)),
        0,
      ) || 0;
    const totalUserPaid =
    ...
    const totalGlobalMeals =
      allMeals.data?.reduce(
        (s, m) => s + (parseFloat(m.day_count) + parseFloat(m.night_count)),
        0,
      ) || 1;
...
async function loadSummary() {
  ...
  try {
    ...
    const allTotalMeals = meals.reduce(
      (sum, m) =>
        sum + (parseFloat(m.day_count || 0) + parseFloat(m.night_count || 0)),
      0,
    );
    ...
    tbody.innerHTML = allMembers
      .map((member) => {
        const memMeals = meals
          .filter((m) => m.member_id === member.id)
          .reduce(
            (sum, m) =>
              sum +
              (parseFloat(m.day_count || 0) + parseFloat(m.night_count || 0)),
            0,
          );
    ...
    allMembers.forEach((member) => {
      const memberMeals =
        meals
          ...
          .reduce(
            (sum, m) =>
              sum + parseFloat(m.day_count) + parseFloat(m.night_count),
            0,
          ) || 0;
